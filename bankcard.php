<?php  
    ...
    if(is_uploaded_file($_FILES['myfile']['tmp_name'])) {  
        ...
        //我们给每个用户动态的创建一个文件夹  
        $user_path=$_SERVER['DOCUMENT_ROOT']."/baiduocr/upload/bankcard".$username;  
        //判断该用户文件夹是否已经有这个文件夹  
    ...
    function ocrstart($filename){
			...
			$url2='upload/bankcard/'.$filename;
			
			
			$json3= json_encode($aipOcr->bankcard(file_get_contents($url2), true), JSON_PRETTY_PRINT);
			...
			$a1=object_array($j3);
			$str1='<h2 style="color:blue;">卡号:'.($a1[result][bank_card_number]).'</h2></br>';
			$str2='<h2 style="color:blue;">银行名称:'.($a1[result][bank_name]).'</h2></br>';
			$str3='<h2 style="color:blue;">银行卡种类:'.($a1[result][bank_card_type]).'</h2></br>';
			$str4='<h2 style="color:pink;">说明：银行卡种类为0代表不能识别; 1代表: 借记卡; 2代表: 信用卡</h2>';
